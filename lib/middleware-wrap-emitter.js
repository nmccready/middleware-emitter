...
module.exports = class MiddlewareWrapEmitter {
  ...
  add(type, name, chain) {
    ...
    const next = compose([this.onerror].concat(chain));

    this.emitter[type](name, (data) => {
      if (data && data instanceof Error) {
        const error = data;
        next.call(this, { req, res, event: { name }, error });
        return;
      }
      req = merge(req, data);
  ...
  /*
    Default (first middleware) overall error handler. Exposed here to
    allow override.
  */
  async onerror(context, next) {
    try {
      await next();
    } catch (e) {
